...
class SingleValueProviderBuilder<T : Any> private constructor(
        path: List<String>,
    ...
    constructor(path: List<String>, clazz: Class<T>) : this(path, clazz, BasicValuePredicateAcceptor())

    @Suppress("UNCHECKED_CAST")
    override fun doBuild(builder: ForgeConfigSpec.Builder): ValueProvider<T> {
        return ConfigValueProvider(
                builder.define(path, { defaultValueSupplier!!.invoke() }, { it as? T != null && predicateAcceptorDelegate.predicate?.invoke(it) != false }, clazz)
        )
    }
}
